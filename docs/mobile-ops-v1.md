...
| PATCH | `/flights/:id` | Whitelisted fields until release locked: `mel`, `other`, `fuelPreviouslyOnboard`, `knownIce`, `otherEnvironment`, `crewId`, `security` (PFR remark), `alternate`, nested `bew`, `jumpseaterObject`, amendments |
| PATCH | `/airport-requirements/:id/runway` | Runway conditions (same as frat web `PATCH /api/airportRequirements/:id` runway fields). **Not** manual METAR / `manualObs` — use frat web for weather observation entry. |
